...
public class Vision extends Subsystem implements RobotMap {
	...
	private double computeDistance(BinaryImage image,
	 ...
	 * 0 for all inputs outside the range 0-2 */
			...
			try {
				...
				for (int i = 0; i < verticalTargetCount; i++) {
					...
					for (int j = 0; j < horizontalTargetCount; j++) {
						...
						if (targetCur.compareTo(bestTarget) > 0)
							bestTarget = targetCur;
					}
				}
				if (verticalTargetCount > 0) {
					...
					ParticleAnalysisReport distanceReport = filteredImage
							.getParticleAnalysisReport(bestTarget.verticalIndex);
					double distance = computeDistance(filteredImage,
							distanceReport, bestTarget.verticalIndex);
					if (bestTarget.isHot()) {
	...
	public static class Scores {
		 ...
		 * equivalent rectangle is the rectangle with sides x and y where
		 * particle area= x*y and particle perimeter = 2x+2y
	...
	public static class TargetReport {
		...
		public final double totalScore, leftScore, rightScore,
				tapeWidthScore, verticalScore;
		public TargetReport(Image filteredImage, int verticalIndex,
			...
			verticalScore = ratioToScore(1
					...
					/ (4 * horizHeight));
			totalScore = Math.max(leftScore, rightScore) + tapeWidthScore
					+ verticalScore;
		}
		...
		public int compareTo(TargetReport o) {
