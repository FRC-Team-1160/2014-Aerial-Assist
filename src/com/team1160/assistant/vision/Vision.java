...
public class Vision extends Subsystem {
	...
	public class Scores {
		double rectangularity, aspectRatioVertical, aspectRatioHorizontal;
	...
	public class TargetReport {
		...
		boolean Hot;
		double totalScore, leftScore, rightScore, tapeWidthScore,
				verticalScore;
	};
	 ...
	 * determine aspect ratio as it performs better as the target gets skewed
	 * by
	 * moving to the left or right. The equivalent rectangle is the rectangle
	 * with sides x and y where particle area= x*y and particle perimeter=
	 * 2x+2y
	...
	public double scoreAspectRatio(BinaryImage image,
		...
		// Vertical reflector 4" wide x 32" tall, horizontal 23.5" wide x 4"
	...
	boolean scoreCompare(Scores scores, boolean vertical) {
		// //OLD: Unoptimized code (looks pretty though)
		// boolean isTarget = true;
		// isTarget &= scores.rectangularity > RobotMap.RECTANGULARITY_LIMIT;
		// if (vertical) {
		// isTarget &= scores.aspectRatioVertical >
		// RobotMap.ASPECT_RATIO_LIMIT;
		// } else {
		// isTarget &= scores.aspectRatioHorizontal >
		// RobotMap.ASPECT_RATIO_LIMIT;
		// }
		// return isTarget;
		if (scores.rectangularity <= RobotMap.RECTANGULARITY_LIMIT)
			return false;
		return (vertical ? scores.aspectRatioVertical
				: scores.aspectRatioHorizontal) > RobotMap.ASPECT_RATIO_LIMIT;
	}
	...
	boolean hotOrNot(TargetReport target) {
		if (target.tapeWidthScore < RobotMap.TAPE_WIDTH_LIMIT) return false;
		if (target.verticalScore < RobotMap.VERTICAL_SCORE_LIMIT)
			return false;
		return (target.leftScore > RobotMap.LR_SCORE_LIMIT)
				|| (target.rightScore > RobotMap.LR_SCORE_LIMIT);
	}
	public void vision() {
		...
		while (autonomous) {
			try {
				...
				for (int i = 0; i < scores.length; i++) {
					...
					scores[i] = new Scores(scoreRectangularity(report),
							scoreAspectRatio(filteredImage, report, i,
									true), scoreAspectRatio(
									filteredImage, report, i, false));
				...
				// Zero out scores and set verticalIndex to first
				// target in case there are no horizontal targets
